...
const CustomerHeader = () => {
 ...
 const { isAddCartSuccessful, setIsAddCartSuccessful } =
 ...
 useEffect(() => {
  if (isAddCartSuccessful) {
   const timer = setTimeout(() => setIsAddCartSuccessful(false), 3500);
   return () => clearTimeout(timer);
  }
 }, [isAddCartSuccessful, setIsAddCartSuccessful]);
 useEffect(() => {
  ...
  <>
   ...
   {isAddCartSuccessful && (
    ...
    >
     ...
     <div>
      <span className="font-medium">Add product to cart successfully!</span>
     </div>
    </div>
   )}
