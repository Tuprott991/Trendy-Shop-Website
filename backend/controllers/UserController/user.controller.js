...
exports.getRetailerList = async (req, res) => {
 try {
  ...
  const retailers = await User.find({ role: "retailer" }).select(
   "name email order_list"
  );
...
exports.getAdminDashboardData = async (req, res) => {
 try {
  ...
  res
   .status(500)
   .json({ message: error.message || "Error fetching dashboard data" });
 }
};
...
exports.manageRetailer = async (req, res) => {
 try {
  ...
  res
   .status(200)
   .json({ message: "Retailers fetched successfully", retailers });
 } catch (error) {
