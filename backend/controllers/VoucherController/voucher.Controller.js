...
exports.postCreateVoucher = async (req, res) => {
 try {
  ...
  if (validFromDate >= validToDate)
   return res
    .status(400)
    .send({ message: "'valid_from' must be before 'valid_to'!" });
...
exports.getVouchersByRetailersID = async (req, res) => {
 try {
  ...
  if (!Array.isArray(retailerIDs) || retailerIDs.length === 0) {
   return res
    .status(400)
    .send({ message: "A list of retailer IDs is required." });
  }
  ...
  if (validRetailerIDs.length === 0) {
   return res
    .status(404)
    .send({ message: "No valid retailers found for the provided IDs." });
  }
  const vouchers = await Voucher.find({
   retailer_id: { $in: validRetailerIDs },
  }).select("code discount_value");
  if (!vouchers.length) {
   return res
    .status(404)
    .send({ message: "No vouchers found for the provided retailers." });
  }
  return res.status(200).json({ vouchers });
 } catch (err) {
  res.status(500).send({
   message:
    err.message ||
    "An error occurred while fetching vouchers for the retailers.",
  });
 }
};
...
exports.applyVoucherToProduct = async (req, res) => {
 try {
  ...
  if (!Array.isArray(products) || products.length === 0 || !voucherCode) {
   return res
    .status(400)
    .send({ message: "products and voucher code are required." });
  }
  ...
  if (now < new Date(voucher.valid_from) || now > new Date(voucher.valid_to)) {
   return res
    .status(400)
    .send({ message: "Voucher is not valid at this time." });
  }
  const updatedCart = products.map((product) => {
   if (String(product.user_id) === String(voucher.retailer_id)) {
  ...
  res.status(500).send({
   message:
    err.message || "An error occurred while applying the voucher to the cart.",
  });
 }
};
