...
const orderSchema = new Schema({
  ...
  timestamps: true,
  statics: {
    async createOrder(retailer_id, total_money, status, address, phone, payment_method, items,vouchers){
      try{
    ...
    async orderInfo(userID){
        try{
          const orderData = await this.find({retailer_id: mongoose.Types.ObjectId(userID)});
          return orderData;
        }
        catch (error){
          console.error('Error get orders information:', error);
    ...
    async countOrders(userID) {
      try {
        const totalOrders = await this.countDocuments({ retailer_id: mongoose.Types.ObjectId(userID) });
        return totalOrders;
      } catch (error) {
